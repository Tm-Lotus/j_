    ...
    async for usr in event.client.iter_participants(event.chat_id):
        ...
        if zilzal.text:
            ...
            try:
                ...
                return
    ZELZAL_BEST_DEV = f"[ᯓ @VIP_ALPOP- اذاعـة خـاص 🚹](t.me/SOURCE_ALPOP) .\n\n**⎉╎تمت الاذاعـه لـ اعضـاء الكـروب .. بنجـاح  ✅**\n**⎉╎عـدد {success} عضـو**"
    ...
    for user in users:
        ...
        if zilzal.text:
            ...
            try:
                ...
                zzz.edit(f"خطـأ فـي إرسـال الرسـالة إلــى {user_entity.id}: {str(e)}")
    ZELZAL_BEST_DEV = f"[ᯓ @VIP_ALPOP- اذاعـة زاجـل 🕊](t.me/SOURCE_ALPOP) .\n⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n**⎉╎تمت الاذاعـه .. بنجـاح  ✅**\n**⎉╎عـدد {success} أشخـاص**"
